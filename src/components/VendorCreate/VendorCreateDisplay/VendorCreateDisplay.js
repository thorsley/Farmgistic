...
class VendorDisplay extends React.Component {
  ...
  handleOpen = (booths) => {
    this.setState({
      open: true,
      activeItemName: booths.farmName,
  ...
  deleteVendor = (booths) => {
    fetch(`https://dcb-market-server.herokuapp.com/booth/${booths.id}`, {
      method: "DELETE",
      headers: new Headers({
        "Content-Type": "application/json",
        Authorization: localStorage.token,
      ...
      <DeleteOutlinedIcon  onClick={()=> window.location.reload(false)} />
        ...
        <div>
          <Modal
            ...
            farmAddress={this.state.activeItemaddress}
